...
- ファイル名が適切か
- **`date`フィールドが変更されていないか**（URL維持のため）

#### 💡 `date`変更時の対応

既存記事を編集する際に`date`フィールドを変更すると、以下の問題が発生します：

- **エラーとして検知され、マージがブロックされます**
- **Gemini Code Assistから修正提案が投稿されます**

**なぜブロックするのか？**
`date`フィールドは記事のURL生成に使用されるため、変更すると既存のリンクが無効になります。

**どうすればよいか？**
Gemini Code Assistが投稿する修正提案（Suggestion）を適用してください：
1. `date`を元の値に戻す
2. `lastupdate`フィールドを追加または更新する

**動作例**:
```yaml
# ❌ date を変更した場合（エラー）
date: 2025-12-12 02:50:11 +09:00  # 元は 2025-12-11
```
...
```yaml
# ✅ 正しい修正
date: 2025-12-11 22:50:11 +09:00  # 元の値に復元
lastupdate: 2025-12-12 02:50:11 +09:00  # 更新日時を記録
```

提案を適用すれば、すぐに検証が通り`/publish`でマージ可能になります。
...
| ⚠️ **date変更エラー** | dateフィールドが変更されている | Geminiの提案を適用してください |
...
`date`フィールドが変更されている場合、提案（suggestion）を生成してマージをブロックします。
