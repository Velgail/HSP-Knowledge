...
既存記事を編集する際、以下の場合に**自動的に修正**されます：

- **`date`フィールドの変更を検知した場合**:
  - `date`を元の値に自動的に戻します
  - `lastupdate`フィールドを現在時刻で更新または追加します
  - 修正内容は自動的にコミット・プッシュされます

> 💡 **理由**: `date`フィールドは記事のURL生成に使用されるため、変更すると既存のリンクが無効になります。記事の更新日時を記録したい場合は、`lastupdate`フィールドを使用してください。

**動作例**:
```yaml
# 変更前（誤って date を変更した場合）
date: 2025-12-12 15:00:00 +09:00  # 元は 2025-12-07
```
...
```yaml
# 自動修正後
date: 2025-12-07 10:00:00 +09:00  # 元の値に復元
lastupdate: 2025-12-12 15:30:45 +09:00  # 現在時刻で追加/更新
```
...
| 🔧 **自動修正を実行しました** | dateフィールドを自動修正 | 修正内容がプッシュされ、再検証されます |
...
- テンプレートのまま残っている箇所
- 投稿者名の不変性（更新時）
- **dateフィールドの変更検知と自動修正（`--fix`オプション）**

**オプション**:
- `--fix`: 自動修正モード。dateフィールドが変更されている場合、元の値に戻し、lastupdateを更新します。
